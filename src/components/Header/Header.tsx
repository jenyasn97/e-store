  ...
  return (
    <header className="header">
      ...
      <nav className="header__nav">
        <Link
          ...
          className={`header__nav-link ${
            isActive("/") && pathname === "/" ? "header__nav-link--active" : ""
          }`}>
          Home
        </Link>
        <Link
      ...
      <div className="header__icons">
        <Link
          to="/favorites"
          className="header__icon header__heart"
          aria-label={`Favorites (${favoriteIds.size} items)`}>
          <svg
            width="28"
            height="26"
            viewBox="0 0 29 26"
            fill={favoriteIds.size > 0 ? "#FF0000" : "none"}
            stroke={favoriteIds.size > 0 ? "#FF0000" : "currentColor"}
            strokeWidth="1.4">
